...
const DepthText = ({
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      activePointer = true;
      // Expanded hit area: 1.6x the element size for smoother enter transition
      const expandFactor = 1.6;
      const cx = rect.left + rect.width / 2;
      const cy = rect.top + rect.height / 2;
      const hw = (rect.width * expandFactor) / 2;
      const hh = (rect.height * expandFactor) / 2;

      const x = clamp((event.clientX - cx) / hw, -1, 1);
      const y = clamp((event.clientY - cy) / hh, -1, 1);
