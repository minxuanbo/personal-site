...
function buildItems(pool: (string | DomeImage)[], seg: number) {
  ...
  const coords = xCols.flatMap((x, c) => {
    ...
    return ys.map(y => ({ x, y, sizeX: 1.6, sizeY: 1.6 }));
  });
...
export default function DomeGallery({
  ...
  useGesture(
    ...
    { target: mainRef as unknown as EventTarget, eventOptions: { passive: true } }
  );
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      lockScroll();
      const parent = el.parentElement!;
      focusedElRef.current = el as HTMLElement;
